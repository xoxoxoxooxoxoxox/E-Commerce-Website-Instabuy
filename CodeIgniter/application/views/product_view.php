      ...
      <?php if ($product):?>
          <div class = "productShowDiv">
            <table class = "productTable">
              <tr>
                <th><a href = "http://cs-server.usc.edu:8436/CodeIgniter/index.php/instabuy/product/<?php echo $product['productID'];?>"><?php echo $product['productName']; ?></a></th>
              </tr>
              <tr>
              ...
              <tr>
                <td>Price: <?php echo '$'.$product['productPrice'] ?></td>
              </tr>
              <?php foreach ($onSales as $rowSales): ?>
                <?php if (($product['productID'] == $rowSales['productID']) && ($rowSales['startDate'] <= $date && ($rowSales['endDate'] >= $date))): ?>
                  <tr>
                    <td style = "color: red; font-size: 1.5em;">Sales Price: <strong><?php $finalPrice = number_format((float)$product['productPrice'] * $rowSales['price'], 2, '.', ''); echo '$'.$finalPrice; ?></strong></td>
                  </tr>
                  <?php $product['productPrice'] = $finalPrice; ?>
                <?php endif; ?>
              <?php endforeach; ?>
